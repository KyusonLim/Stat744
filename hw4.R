...
### BMB: don't do this. It implies you're not running a clean pipeline (i.e., restarting from a clean R session)
## rm(list=ls())
...
## BMB: this is cute, but it's _definitely_ not an inferential plot.
...
# However, with shiny interactive app from official statistics Canada for how many COVID-19 people dies we could understand quantitatively for the effect of COVID-19 by the data mining models and techniques.


## BMB: I'm still not sure what this tells us.  In this particular case,
## (1) I would probably filter out "covid", "19", and possibly "canada", "canadian" from the corpus; (2) I'm not sure this corpus is big enough to be useful (there are only 24 distinct terms, with 346 total occurrences
...
## BMB: once again, I'm not sure that this visualization is worth the effort. You've managed to draw a figure that reflects only 6 data points (why don't I see the smaller provinces?) Should we be viewing per capita mortality rather than total mortality?
...
## BMB: mark 2/3. There's a lot of fancy technical stuff here, but neither of these plots really addresses a scientific or inferential question ("how many deaths are there in each province?" isn't a particularly scientific question).  I also want to emphasize that if you used any code from online sources, even if you adapted it a lot to your example, you should definitely cite those sources. (Also, I'm not sure that scraping the covid-19 web page and plotting the results constitutes "geospatial data mining"; doesn't the covid-19 page give you a way to download the data directly without scraping it?)
